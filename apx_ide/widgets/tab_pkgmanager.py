...
class TabPkgManager(Gtk.Box):
    ...
    def __on_delete_clicked(self, button: Gtk.Button) -> None:
        def on_callback(result, *args) -> None:
            ...
            if status:
                self.__window.toast(_(f"{self.__pkgmanager.name} package manager deleted"))
                self.__window.remove_pkgmanager(self.__aid)

        def on_response(dialog: Adw.MessageDialog, response: str) -> None:
            if response == "ok":
                self.__window.toast(_(f"Deleting {self.__pkgmanager.name} package manager..."))
                RunAsync(self.__pkgmanager.remove, on_callback, force=True)
            dialog.destroy()

        dialog: Adw.MessageDialog = Adw.MessageDialog.new(
            self.__window,
            _(f"Are you sure you want to delete the {self.__pkgmanager.name} package manager?"),
            _("This action will delete the package manager and all its data. This action cannot be undone."),
        )
